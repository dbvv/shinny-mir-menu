...
class ControllerExtensionModuleMegamenu extends Controller {
    ...
    public function form() {
        ...
        $data['text_limit_item']                = $this->language->get('text_limit_item');
        ...
        } elseif (!empty($item['limit_item'])) {
            $data['limit_item'] = $item['limit_item'];
        } else {
            $data['limit_item'] = 5;
        }
        ...
        if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
            ...
            $data['thumb'] = $this->model_tool_image->resize($item['thumb'], 100, 100);
        } else {
    ...
    protected function validate() {
        ...
        if (!in_array($this->request->post['menu_type'], array(
            "category",
            "html",
            "manufacturer",
            "information",
        ...
        //category
        if ($this->request->post['menu_type'] == "category" && !in_array($this->request->post['variant_category'], array(
            "simple",
            "full",
            "full_image"
        ))) {
    ...
    public function refresh() {
        ...
        if ($this->checkIfColumnExist(DB_PREFIX . "megamenu", 'link')) {
            $this->db->query("ALTER TABLE `" . DB_PREFIX . "megamenu` DROP COLUMN `link`");
        }

        if (!$this->checkIfColumnExist(DB_PREFIX . "megamenu_description", 'link')) {
            $this->db->query("ALTER TABLE  `" . DB_PREFIX . "megamenu_description` ADD  `link` varchar(255) AFTER  `title`");
        }

        if (!$this->checkIfColumnExist(DB_PREFIX . "megamenu", 'limit_item')) {
            $this->db->query("ALTER TABLE  `" . DB_PREFIX . "megamenu` ADD `limit_item` int(11) NOT NULL DEFAULT '5' AFTER `menu_type`");
        }
