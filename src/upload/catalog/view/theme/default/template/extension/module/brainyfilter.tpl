...
<style type="text/css">
   ...
   .bf-slider-range span:last-child {
...
<div class="product-search bf-panel-wrapper<?php if($isResponsive) : ?> bf-responsive<?php endif; ?> <?php echo $responsivePos; ?> bf-layout-id-<?php echo $layout_id;?>">
            ...
            <form class="bf-form 
                ...
                <?php foreach ($filters as $i => $section) : ?>
                    ...
                    <?php if ($section['type'] == 'price') : ?>
                        ...
                        <div class="bf-attr-block bfv<?php echo $i; ?> bf-price-filter <?php if ($isHorizontal && isset($filters[$i + 1]) && $filters[$i + 1]['type'] === 'search') : ?>bf-left-half<?php endif; ?>">
                    ...
                    <?php elseif ($section['type'] == 'search') : ?>
                        ...
                        <div class="bf-attr-block bfv<?php echo $i; ?> bf-keywords-filter <?php if ($isHorizontal && isset($filters[$i + 1]) && $filters[$i + 1]['type'] === 'price') : ?>bf-left-half<?php endif; ?>">
                        <div class="bf-attr-header<?php if ($section['collapsed']) : ?> bf-collapse<?php endif; ?><?php if (!$i) : ?> bf-w-line<?php endif; ?>">
                    ...
                    <?php elseif ($section['type'] == 'category') : ?>
                        ...
                        <div class="bf-attr-block bfv<?php echo $i; ?>">
                        <div class="bf-attr-header<?php if ($section['collapsed']) : ?> bf-collapse<?php endif; ?><?php if (!$i) : ?> bf-w-line<?php endif; ?>">
                    ...
                    <?php else : ?>
                        ...
                        <?php foreach ($section['array'] as $groupId => $group) : ?>
                            ...
                            <div class="bfv<?php echo $i; ?> bf-attr-block<?php if (in_array($group['type'], array('slider', 'slider_lbl', 'slider_lbl_inp'))) : ?> bf-slider<?php endif; ?>">
                            <div class="bf-attr-header<?php if ($section['collapsed']) : ?> bf-collapse<?php endif; ?><?php if (!$i) : ?> bf-w-line<?php endif; ?>">
                                <?php echo $group['name']; ?>
                            </div>
                            <div class="bf-attr-block-cont">
                                ...
                                <?php else : ?>
                                    ...
                                    <?php foreach ($group['values'] as $value) : ?>
                                    <?php $valueId  = $value['id']; ?>
                                    <div class="mani<?php echo $value['id']; ?> bf-attr-filter bf-attr-<?php echo $groupUID; ?> bf-row <?php 
                                    if (isset($totals) && isEmptyStock($groupUID, $valueId, $postponedCount, $totals, $selected) && $settings['behaviour']['hide_empty']):
                                        ...
                                        <span class="bf-cell bf-c-1">
                                            <input data-idd="<?php echo $groupUID; ?>" id="bf-attr-<?php echo $groupUID . '_' . $valueId . '_' . $layout_id; ?>"
                                                   data-filterid="bf-attr-<?php echo $groupUID . '_' . $valueId; ?>"
                                                   type="<?php echo $group['type']; ?>" 
                                                   name="<?php echo "bfp_{$groupUID}"; ?><?php if ($group['type'] === 'checkbox') { echo "_{$valueId}"; } ?>"
                                                   value="<?php echo $valueId; ?>" 
                                                   <?php if (isset($selected[$groupUID]) && in_array($valueId, $selected[$groupUID])) : ?>checked="true"<?php endif; ?> class="bff" />
                                        </span>
...
<?php endif; ?>
<script src="catalog/view/javascript/jquery/jquery.mCustomScrollbar.concat.min.js"></script>
<script type="text/javascript">
    $('.bf-attr-header').each(function(i, e){
            setTimeout(function(){
            $(e).next().find('.bf-sliding').mCustomScrollbar({
                theme:"dark-thin"
            });
            }, 1700);
    });
    if ($(window).width() < 768) {
     ...
     $(document).on('click', '.bf-clickable', function(){
        $(this).next().find('.bf-sliding').css('height','250px').mCustomScrollbar({
                theme:"dark-thin"
            });
        $(this).addClass('heid');
    });
    $(document).on('click', '.heid', function(){
        $(this).next().find('.bf-sliding').css('height','0');
        $(this).removeClass('heid');
    });
    } else {
    $(document).on('click', '.bf-clickable', function(){
        $(this).next().find('.bf-sliding').css('height','0').mCustomScrollbar({
                theme:"dark-thin"
            });
        $(this).addClass('heid');
    });
    $(document).on('click', '.heid', function(){
        $(this).next().find('.bf-sliding').css('height','250px');
        $(this).removeClass('heid');
    });
    }
</script>